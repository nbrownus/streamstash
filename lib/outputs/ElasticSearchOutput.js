...
ElasticSearchOutput.prototype._finishRequest = function (data, queue) {
    ...
    try {
        ...
        for (var i = queue.items.length - 1; i >= 0; i--) {
            if (badIds.hasOwnProperty(queue.items[i].eventId) === true) {
                self.logger.error(
                    ...
                    'Error:', JSON.stringify(badIds[queue.items[i].eventId]), ';',
                    'event:', JSON.stringify(queue.items[i].data)
                )
        ...
        self.logger.error(self.name, 'Error consuming elasticsearch response', error.stack, data)
