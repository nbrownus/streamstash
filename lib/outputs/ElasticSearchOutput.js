...
ElasticSearchOutput.prototype._performPost = function (forced) {
    ...
    for (var eventId in queue.items) {
        ...
        var item = queue.items[eventId],
            type = item.event.data[self.typeField] || self.defaultType,
            timestamp = item.event.data[self.timestampField] || new Date(),
            dateString = timestamp.getFullYear(),
            month = timestamp.getMonth() + 1,
            day = timestamp.getDate(),
            useData = util._extend({}, item.event.data)

        dateString += '.' + ((String(month).length == 1) ? '0' + month : month)
        dateString += '.' + ((String(day).length == 1) ? '0' + day : day)

        write += JSON.stringify({
            index: { '_index': self.indexPrefix + '-' + dateString, '_type': type, '_id': item.id }
        }) + '\n'
