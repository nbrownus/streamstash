 ...
 * @param {object} [options.agentConfig] Config options to pass to the http or https agent {@see https.Agent} or {@see http.agent}
...
var ElasticSearch = function (options) {
    ...
    useOptions.agentConfig = useOptions.agentConfig || { maxSockets: 1000, rejectUnauthorized: true }
    ...
    if (useOptions.tls) {
        self.agent = new https.Agent(useOptions.agentConfig)
        self.driver = https
    } else {
        self.agent = new http.Agent(useOptions.agentConfig)
        self.driver = http
    }
    self.agent.maxSockets = useOptions.agentConfig.maxSockets
...
ElasticSearch.prototype._performPost = function (forced) {
    ...
    request.on('error', function (error) {
        self.logger.log(self.name, 'Error during request', error.stack)
    })
