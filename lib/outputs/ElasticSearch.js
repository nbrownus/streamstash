...
//TODO This isn't super cool, new up our own agent
...
ElasticSearch.prototype._handleOutput = function (eventContainer) {
    ...
    if (!this.queue.cache['logstash-' + dateString]) {
        this.queue.cache['logstash-' + dateString] = {}
    }

    if (!this.queue.cache['logstash-' + dateString][type]) {
        this.queue.cache['logstash-' + dateString][type] = []
    }

    this.queue.items.push(eventContainer)
    this.queue.cache['logstash-' + dateString][type].push(JSON.stringify(write))
...
ElasticSearch.prototype._performPost = function (forced) {
    ...
    if (self.queue.items.length < 100 && !forced) {
        if (!self.queue.timer && self.queue.items.length > 0) {
            self.queue.timer = setTimeout(function () { self._performPost(true) }, 1000)
        }
    ...
    for (var index in queue.cache) {
        for (var type in queue.cache[index]) {
            write.push(JSON.stringify({ "index" : { "_index" : index, "_type" : type }}))

            //TODO: concat
            for (var item in queue.cache[index][type]) {
                write.push(queue.cache[index][type][item])
            }
        }
    }
