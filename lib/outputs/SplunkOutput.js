 ...
 * @param {Boolean} [options.tls=true] Whether or not to use TLS (true or false, default=true, false is disabled!)
...
let Splunk = function (options) {
    ...
    if (useOptions.tls) {
        ...
        throw new Error('Non TLS connections are disabled')
...
SplunkOutput.prototype._performPost = function (forced) {
    ...
    for (let eventId in queue.items) {
        ...
        let header = { "sourcetype": "_json", "event": { "_index": self.indexPrefix + '-' + dateString, "_type": type } }

        if (self.idField && event.data.hasOwnProperty(self.idField)) {
            header['event']['_id'] = event.data[self.idField]
        }
