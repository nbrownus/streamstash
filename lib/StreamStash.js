...
StreamStash.prototype._handleInputEvent = function (plugin, eventContainer) {
    ...
    if (!eventContainer.data.hasOwnProperty('event_source')) {
        self.logger.error('Event received from', plugin.name, 'has no event_source property')
    }
...
StreamStash.prototype._doFilter = function (index, eventContainer) {
    ...
    var canAct = function () {
        if (nexted) {
            self.logger.error('Filter', filter.name, 'tried to call `next` on an event that has already progressed')
            self.logger.info(eventContainer)
            return false
        }

        if (doned) {
            self.logger.error('Filter', filter.name, 'tried to call `done` on an event that has already progressed')
            self.logger.info(eventContainer)
            return false
        }
        ...
        if (canceled) {
            self.logger.error('Filter', filter.name, 'tried to call `cancel` on an event that has already progressed')
            self.logger.info(eventContainer)
            return false
        }
...
StreamStash.prototype._handleProcessingComplete = function (eventContainers) {
    ...
    for (var i = 0, len = events.length; i < len; i++) {
        ...
        self.logger.debug('Event', eventContainer.eventId, 'processing', stateStr)
    }
