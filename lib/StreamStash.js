...
var StreamStash = function (options) {
    ...
    self._telemetryInterval = setInterval(
        function () {
            ...
            self.telemetry.gauge('process.memory.rss', mem.rss)
            self.telemetry.gauge('process.memory.heap_total', mem.heapTotal)
            self.telemetry.gauge('process.memory.heap_used', mem.heapUsed)

            var v8mem = v8.getHeapStatistics()
            for (var stat in Object.keys(v8mem)) {
                self.telemetry.gauge('v8mem.' + stat, v8mem[stat])
            }
        },
