 ...
 * @param {EventContainer} eventContainer The event that was output
 *
 * @private
 */
StreamStash.prototype._handleOutputComplete = function (output, eventContainer) {
    if (eventContainer._plugins[output.pluginId]) {
        this.logger.error('Output plugin', output.name, 'completed the same event twice')
        this.logger.debug(eventContainer)
        return
    }
    ...
    this.logger.debug(output.name, 'completed output.', eventContainer._pluginsLeft, 'plugins left')
    if (eventContainer._pluginsLeft === 0) {
        this._handleProcessingComplete(eventContainer, false)
    }
}
 ...
 *
 * @param {EventContainer} eventContainer The event that has finished processing
 * @param {Boolean} canceled True if the event was canceled, false if not
 *
 * @private
 */
StreamStash.prototype._handleProcessingComplete = function (eventContainer, canceled) {
    ...
    this.logger.debug('Event', eventContainer.eventId, 'processing', canceled ? 'canceled' : 'completed')
    this.logger.debug(eventContainer)

    this.stats.events.processing--

    if (this.state === StreamStash.STATE.STOPPING_INPUT) {
        this._attemptStop()
    }
}
