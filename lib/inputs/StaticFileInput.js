...
var StaticFileInput = function (options) {
    ...
    rs.on('end', function () {
        self.logger.info(self.name, 'finished consuming the file', { file: options.file })
    })

    rs.on('error', function (error) {
        self.logger.error(self.name, 'error while consuming the file', { file: options.file, error: error.stack || error })
    })
