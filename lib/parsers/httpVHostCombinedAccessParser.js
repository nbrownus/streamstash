var regex = /^([^:]+:\d+) (\S+) (\S+) (\S+) \[([^\]]+)] "(\S+) (\S+) (\S+)" (\d+) (\d+) "([^"]*)" "([^"]*)"$/,
    fields = ['virtual_host', 'remote_host', 'identd', 'user', 'timestamp', 'method', 'path', 'version', 'status_code', 'bytes', 'referrer', 'user_agent']

/**
 * Parses the common vhost combined access log lines that an httpd outputs
 * Its the combined log format with virtual_host:local_port on the front
 *
...
module.exports.propertyName = 'http_vhost_access'
