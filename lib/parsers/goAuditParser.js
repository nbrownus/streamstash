...
//TODO: attach unknown fields to the data object so it can be understood what is not being fully parsed
...
var parseSyscall = function (msgs, result, uidMap) {
    ...
    result.data.syscall.command = convertValue(result.data.syscall.comm || '', true)
    delete result.data.syscall.comm

    result.data.syscall.executable = convertValue(result.data.syscall.exe || '', true)
...
var parseProctitle = function (msgs, result) {
    ...
    result.data.proctitle = convertValue(data.proctitle || '', true)
}
...
var parseAddr = function (str) {
    if (str.length < 2) {
        return { unknown: str }
    }

    var family = parseInt(str.substr(0, 2), 16) + (256 * parseInt(str.substr(2, 2), 16))

    if (!constants.addressFamilies.hasOwnProperty(family)) {
        return { unknown: str }
    }

    var details =  {
        family: constants.addressFamilies[family],
    }

    switch (family) {
        case 1: // unix/local/file
            parseAddrLocal(str, details)
            break

        case 2: // inet
            parseAddrInet(str, details)
            break

        case 10: // inet6
            parseAddrInet6(str, details)
            break

        default:
            details.unknown = str.substr(4)
    }

    return details
}

var parseAddrInet = function (str, details) {
    if (str.length < 16) {
        details.unknown = str.substr(2)
        return
    }

    details.port = (parseInt(str.substr(4, 2), 16) * 256) + parseInt(str.substr(6, 2), 16)

    details.ip = parseInt(str.substr(8, 2), 16) + '.' +
        parseInt(str.substr(10, 2), 16) + '.' +
        parseInt(str.substr(12, 2), 16) + '.' +
        parseInt(str.substr(14, 2), 16)

    if (str.length > 16) {
        details.unknown = str.substr(16)
    }
}

var parseAddrInet6 = function (str, details) {
    if (str.length < 56) {
        details.unknown = str.substr(2)
        return
    }
    ...
    details.flow_info = str.substr(8, 8)

    details.ip = str.substr(16, 4).toLowerCase() + ':' +
        str.substr(20, 4).toLowerCase() + ':' +
        str.substr(24, 4).toLowerCase() + ':' +
        str.substr(28, 4).toLowerCase() + ':' +
        str.substr(32, 4).toLowerCase() + ':' +
        str.substr(36, 4).toLowerCase() + ':' +
        str.substr(40, 4).toLowerCase() + ':' +
        str.substr(44, 4).toLowerCase()

    //TODO: Figure out translation
    details.scope_id = str.substr(48, 8)

    if (str.length > 56) {
        details.unknown = str.substr(56)
    }
}

var parseAddrLocal = function (str, details) {
    if (str.length < 5) {
        details.unknown = str.substr(2)
        return
    }
    ...
    var endPos = str.indexOf('00', 4) - 4
    //TODO: sometimes the path is null prefixed!
    if (endPos < 0) {
        endPos = str.length - 4
    }

    details.path = convertValue(str.substr(4, endPos), true)

    if (str.length > endPos + 5) {
        details.unknown = str.substr(endPos + 4)
    }
}
...
var buildMessage = function (result) {
    ...
    if (data.hasOwnProperty('syscall')) {
        if (data.syscall.hasOwnProperty('auid') && data.syscall.auid && data.syscall.auid.id !== data.syscall.uid.id) {
            message += data.syscall.auid.name + ' as '
        }

        // Who did it?
        if (data.syscall.hasOwnProperty('uid') && data.syscall.uid) {
        ...
        if (data.hasOwnProperty('execve') && data.execve.hasOwnProperty('command')) {
            ...
            switch (data.syscall.name) {
                ...
                case 'bind':
                    ...
                    if (data.hasOwnProperty('socket_address')) {
                        if (data.socket_address.hasOwnProperty('ip')) {
                            message += '`' + data.socket_address.ip + ':' + data.socket_address.port + '` '
                        } else if (data.socket_address.hasOwnProperty('path')) {
                            message += '`' + data.socket_address.path + '` '
                        } else {
                            //TODO: put the family in here
        ...
        if (includeCmd && data.syscall.command) {
            message += 'as `' + data.syscall.command + '` '
        }
    }
...
var getPathName = function (path)  {
    ...
    return path.name || 'inode: ' + path.inode
}

var findPathType = function (paths, type) {
    for (var i in paths) {
        if (paths[i].nametype === type || paths[i].objtype === type) {
            return paths[i]
        }
    }
}
